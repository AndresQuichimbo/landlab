...
def find_drainage_area_and_discharge_to_n_lossy_TL(
	s, r, l, p, total_donors, loss_function, grid, node_cell_area=1.0, runoff=1.0, boundary_nodes=None
	):
	...
	drainage_area = numpy.zeros(np) + node_cell_area
	discharge = numpy.zeros(np) + node_cell_area * runoff
	distance = grid.at_node["distance"]
	irecvr_number_of_donors=[[]]*np
	# grab the field to ouput loss to
		...
		for v in range(q):
			...
			if proportion > 0:
				if donor != recvr:
					...
					distance_donor_recvr[recvr].append(distance[donor])
					...
					if len(irecvr_number_of_donors[recvr])==total_donors[recvr]:
						...
						if len(numpy.where(irecvr_number_of_donors[recvr]>0.0)[0])==1:
							# If there is just one donor different of zero add distance
							# to the receiver, if all are zero keep the same distance
							donor_distance_id=numpy.where(irecvr_number_of_donors[recvr]>0.0)[0]
							distance[recvr]+=drainage_area[donor_distance_id]
