...
class LossyFlowAccumulator(FlowAccumulator):
	...
	**kwargs : optional
	...
	def _accumulate_A_Q_to_one_dis(self, s, r):
		"""Accumulate area and discharge for a route-to-one scheme."""
		link = self._grid.at_node["flow__link_to_receiver_node"]
		total_donors=[]
		for i in range(len(r)):
			total_donors.append(len(numpy.where(numpy.array(r)==i)[0]))

		a, q = flow_accum_bw.find_drainage_area_and_discharge_lossy_TL(
			s,
			r,			
			link,
			total_donors,
	...
	def _accumulate_A_Q_to_n(self, s, r, p):
		...
		total_donors=[]
		for i in range(len(r)):
			total_donors.append(len(numpy.where(r==i)))
