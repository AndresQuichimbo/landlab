...
def find_drainage_area_and_discharge_lossy_TL(
	s, r, l, total_donors, loss_function, grid, node_cell_area=1.0, runoff=1.0, boundary_nodes=None
	):
	...
	#distance = numpy.zeros(np)+grid.at_node["distance"][:]
		
	# number of donors per of each cell
	
	irecvr_number_of_donors=[]
	distance_donor_recvr=[]
	Q0_donor_recvr=[]
	for ik in range(np):
		distance_donor_recvr.append([])
		irecvr_number_of_donors.append([])
		Q0_donor_recvr.append([])
	#print(distance_donor_recvr)
    # note no loss occurs at a node until the water actually moves along a link
	...
	for i in range(np - 1, -1, -1):
		...
		if donor != recvr:
			...
			# save donors distance until all donors of receiver are completed
			...
			distance_donor_recvr[recvr].append(grid.at_node["distance"][donor])
			
			Q0_donor_recvr[recvr].append(grid.at_node["ini_flow"][donor])
			
			irecvr_number_of_donors[recvr].append(number_of_donors)
			#print(recvr,total_donors[recvr],irecvr_number_of_donors[recvr])
			# Check if total number of nodes has been reached, if not
			# keep accumulating until it is reached
			#print(recvr,len(irecvr_number_of_donors[recvr]),total_donors[recvr])
			#print(total_donors)
			if len(irecvr_number_of_donors[recvr])==total_donors[recvr]:
				# Find location of the donors with discharge different of zero
				#print(irecvr_number_of_donors[recvr])
				#print(len(numpy.where(numpy.array(irecvr_number_of_donors[recvr])>0)))
				if len(numpy.where(numpy.array(irecvr_number_of_donors[recvr])>0)[0])==1:
					# If there is just one donor different of zero add distance
					# to the receiver, if all are zero keep the same distance
					#print(irecvr_number_of_donors[recvr])
					donor_distance_id=numpy.where(numpy.array(irecvr_number_of_donors[recvr])>0)[0]
					#print('bf',recvr,donor,donor_distance_id[0],grid.at_node["distance"][recvr],distance_donor_recvr[recvr][donor_distance_id[0]])
					if grid.at_node["runoff"][recvr]==0:
						grid.at_node["ini_flow"][recvr]=Q0_donor_recvr[recvr][donor_distance_id[0]]
						grid.at_node["distance"][recvr]+=distance_donor_recvr[recvr][donor_distance_id[0]]
					#print(grid.at_node["distance"][recvr],distance_donor_recvr[recvr][donor_distance_id])
					#print('af',recvr,donor,donor_distance_id[0],distance[recvr],distance_donor_recvr[recvr][donor_distance_id[0]])
			

	return drainage_area, discharge#, distance
